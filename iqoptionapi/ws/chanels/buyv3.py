...
class Buyv3_by_raw_expired(Base):
    ...
    def __call__(self, price, active, direction, option, expired, request_id):
        ...
        data = {
            ...
            "name": "binary-options.open-option",
            "version": "2.0" if option_id == 12 else "1.0"
        }
        
        if option_id == 12:
            data["body"]["refund_value"] = 0
            data["body"]["value"] = 0
            data["body"]["profit_percent"] = 0
            
            # For blitz, we need expiration_size. We can estimate it from the expired time
            # Or if it's 5s blitz, we just put 5. Let's put 30 if expired > timesync + 10 else 5.
            try:
                server_time = int(self.api.timesync.server_timestamp)
                diff = int(expired) - server_time
                if diff > 15:
                    exp_size = 30
                else:
                    exp_size = 5
            ...
            data["body"]["expiration_size"] = exp_size
