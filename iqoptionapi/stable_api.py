...
class IQ_Option:
    ...
    def __init__(self, email, password, active_account_type="PRACTICE"):
        ...
        self.SESSION_COOKIE = {}
        self.get_candles_lock = threading.Lock()
        #
        # --start
        # self.connect()
    ...
    def connect(self, sms_code=None):
        ...
        if check == True:
            ...
            print("Falha na conexao: " + str(reason))
            print("resultado do json: " + str(json.loads(reason)))
    ...
    # _________________________self.api.get_api_option_init_all_v2() wss______________________
    ...
    def __get_binary_open(self):
        ...
        binary_data = self.get_all_init()
        # Incluindo blitz na lista de processamento
            ...
            for option in binary_list:
                if option in binary_data:
                    for actives_id in binary_data[option]["actives"]:
                        ...
                        # Proteção para nomes que não seguem o padrão "front.NAME"
                        if "." in str(active["name"]):
                            name = str(active["name"]).split(".")[1]
                        else:
                            name = str(active["name"])
                            
                        if active["enabled"] == True:
                            if active.get("is_suspended") == True:
        ...
        for dirr in ["binary", "turbo", "blitz"]:
            if binary_data and dirr in binary_data:
    ...
    # --------for profit (obsolete for binary, kept for compatibility if needed)
    def get_all_profit(self):
        all_profit = nested_dict(2, dict)
        init_info = self.get_all_init()
        if not init_info: return all_profit
        
        for option_type in ["turbo", "binary", "blitz"]:
            if option_type in init_info:
                for actives in init_info[option_type]["actives"]:
                    active_data = init_info[option_type]["actives"][actives]
                    if "." in active_data["name"]:
                        name = active_data["name"].split(".")[1]
                    else:
                        name = active_data["name"]
                    
                    try:
                        all_profit[name][option_type] = (100.0 - active_data["option"]["profit"]["commission"]) / 100.0
                    except:
                        pass
        return all_profit
    ...
    def get_candles(self, ACTIVES, interval, count, endtime):
        ...
        import threading
        req_id = str(int(time.time() * 1000000)) + str(id(threading.current_thread()))

        while True:
            try:
                self.api.getcandles(OP_code.ACTIVES[ACTIVES], interval, count, endtime, request_id=req_id)

                # Adiciona um pequeno atraso para evitar busy-waiting
                start_time = time.time()
                while self.check_connect():
                    if hasattr(self.api.candles, "candles_data_dict") and req_id in self.api.candles.candles_data_dict:
                        return self.api.candles.candles_data_dict.pop(req_id)
...
##############################################################################################
    ...
    def buy_by_raw_expirations(self, price, active, direction, option, expired):
        ...
        self.api.buyv3_by_raw_expired(
            price, OP_code.ACTIVES[active], direction, option_id, expired, request_id=req_id)
        start_t = time.time()
        id = None
...
# __________________for Digital___________________
    ...
    def get_realtime_strike_list(self, ACTIVE, duration):
        while True:
            if not self.api.instrument_quotes_generated_data[ACTIVE][duration * 60]:
                pass
            else:
                break
    ...
    def buy_digital(self, amount, active, action, duration):
        # Versão simplificada e mais estável de compra Digital
        action = action.lower()
        if action == 'put':
            action = 'P'
        elif action == 'call':
        ...
        if not asset_id and not active.endswith("-OTC"):
            asset_id = OP_code.ACTIVES.get(active + "-op")
            
        if not asset_id:
            return False, "Asset ID not found for Digital"
            
        instrument_id = "do" + str(asset_id) + "A" + \
            date_formated[:8] + "D" + date_formated[8:] + \
            "00T" + str(duration) + "M" + action + "SPT"
        
        request_id = self.api.place_digital_option_v2(instrument_id, asset_id, amount)
    ...
    def buy_blitz(self, amount, active, action, duration):
        # O Blitz usa o sistema de ordens binárias mas com o ID do ativo Blitz
        # A expiração é o tempo atual + a duração em segundos
        timestamp = int(self.api.timesync.server_timestamp)
        expired = timestamp + duration
        
        asset_id = OP_code.ACTIVES.get(active)
        if not asset_id:
            return False, "Asset ID not found for Blitz"
            
        # O option_type_id para Blitz é 16
        return self.buy_by_raw_expirations(amount, active, action, "blitz", expired)
    ...
    def buy_digital_by_id(self, amount, instrument_id):
        request_id = self.api.place_digital_option(instrument_id, amount)
        start_t = time.time()
        while self.api.digital_option_placed_id.get(request_id) == None:
            if time.time() - start_t > 30:
                ...
                return False, None
            time.sleep(0.1)
        return True, self.api.digital_option_placed_id.get(request_id)
